...
class PacketSenderController:
    ...
    def show_dialog(self, selected_device: Optional[Any] = None) -> None:
        """显示数据包发送对话框
        ...
        # 直接加载UI文件到一个新的对话框
        dialog = cast(QWidget, loader.load(ui_file, self.main_window))
        ui_file.close()
        ...
        # 显示对话框
        if hasattr(self.dialog, "exec_"):
            self.dialog.exec_()

    def _init_ui(self, selected_device: Optional[Any] = None) -> None:
        """初始化界面
    ...
    def _setup_packet_tree(self) -> None:
        """设置数据包树"""
        # 确保数据包树视图存在
    ...
    def _connect_signals(self) -> None:
        ...
        if self.cancelButton is not None and self.dialog is not None:
            if hasattr(self.dialog, "reject"):
    ...
    def _send_packet(self) -> None:
        ...
        # 只有当dialog存在时才尝试关闭它
        if self.dialog and hasattr(self.dialog, "accept"):
            self.dialog.accept()

    def create_protocol_tree_model(self) -> QStandardItemModel:
        """创建协议树模型

...
class PkjvizMainWindow(QMainWindow):
    ...
    def set_app_mode(self, mode: int) -> None:
        ...
        if mode == MODE_OFFLINE:
            ...
            # 隐藏发包工具
            if hasattr(self.ui, "packetSenderWidget"):
                self.ui.packetSenderWidget.setVisible(False)
    ...
    def _create_packet_sender_widget(self) -> None:
        """创建发包工具组件并添加到编辑区下方"""
        # 导入需要的模块
        from PySide6.QtWidgets import (
        ...
        # 创建发包工具容器
        packet_sender_widget = QWidget()
        self.ui.packetSenderWidget = packet_sender_widget  # 存储引用
        main_layout = QVBoxLayout(packet_sender_widget)
        
        # 设置样式
        packet_sender_widget.setStyleSheet("""
            QWidget {
        ...
        open_button.clicked.connect(lambda: self.log_message("打开数据包"))
        ...
        show_button.clicked.connect(lambda: self.log_message("显示Hex数据"))
...
00 00 00 00 00 00 00 00 00 00 00 00 00"""
        ...
        cancel_button.clicked.connect(lambda: packet_sender_widget.setVisible(False))
        ...
        # 添加到主界面的编辑器分割器下方
        editor_splitter = self.ui.findChild(QSplitter, "editorSplitter")
        if editor_splitter:
            editor_splitter.addWidget(packet_sender_widget)

...
class RegisterEditor(QWidget):
    ...
    def update_filter(self) -> None:
        ...
        show_modified = self.show_modified_checkbox.isChecked() if self.show_modified_checkbox else False
        ...
        def filter_func(row: int, parent_index: QModelIndex) -> bool:
            ...
            # 地址过滤
            if address_filter and address_filter.lower() not in str(source_model.data(source_model.index(row, 0))).lower():
                return False
                
            # 名称过滤
            if name_filter and name_filter.lower() not in str(source_model.data(source_model.index(row, 1))).lower():
                return False
                
            # 描述过滤
            if desc_filter and desc_filter.lower() not in str(source_model.data(source_model.index(row, 3))).lower():
                return False
    ...
    def update_value_displays(self) -> None:
        """更新值的不同显示格式"""
        if not self.value_edit or not self.dec_value_label or not self.binary_value_label:
            return
    ...
    def on_selection_changed(self) -> None:
        ...
        # 获取寄存器数据
        register = cast(RegisterTableModel, self.proxy_model.sourceModel()).getRegisterByIndex(row)
        ...
        if register is not None:
            ...
            if self.value_edit:
                self.value_edit.setReadOnly(not register.writable and not self.editing_new_register)
            if self.description_edit:
    ...
    def on_import_clicked(self) -> None:
        ...
        register_group = self.register_group_combo.currentText() if self.register_group_combo else ""
    ...
    def on_delete_clicked(self) -> None:
        ...
        if reply == QMessageBox.StandardButton.Yes:
            # 从模型中删除
            self.register_model.beginRemoveRows(QModelIndex(), self.current_selected_index, self.current_selected_index)
    ...
    def on_apply_clicked(self) -> None:
        """应用按钮点击事件"""
        if not self.address_edit or not self.name_edit or not self.value_edit or not self.description_edit or not self.writable_checkbox:
            return
        ...
        if self.editing_new_register:
            ...
            # 更新现有寄存器
            reg = cast(RegisterTableModel, self.proxy_model.sourceModel()).getRegisterByIndex(self.current_selected_index)
    ...
    def on_registers_saved(self) -> None:
        """寄存器保存成功事件"""
        if self.status_label:
            self.status_label.setText(f"已保存到文件: {self.register_controller.current_file_path}")
    ...
    def on_registers_loaded(self) -> None:
        ...
        if self.register_controller.current_file_path:
            self.status_label.setText(f"已从文件加载: {self.register_controller.current_file_path}")
        else:
